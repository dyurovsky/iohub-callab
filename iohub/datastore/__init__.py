...
class ioHubpyTablesFile():
        ...
        for event_cls_name,event_cls in event_class_dict.iteritems():
            if event_cls.IOHUB_DATA_TABLE:
                ...
                if event_table_label not in self.TABLES.keys():
                    try:
                        self.TABLES[event_table_label]=self.emrtFile.create_table(self._eventGroupMappings[event_table_label],eventTableLabel2ClassName(event_table_label),event_cls.NUMPY_DTYPE, title="%s Data"%(device_instance.__class__.__name__,),filters=dfilter.copy())
                        self.flush()
                        ...
                        self.TABLES[event_table_label]=self._eventGroupMappings[event_table_label]._f_get_child(eventTableLabel2ClassName(event_table_label))# self.emrtFile.create_table(,eventTableLabel2ClassName(event_table_label),event_cls.NUMPY_DTYPE, title="%s Data"%(device_instance.__class__.__name__,),filters=dfilter.copy())
    ...
    def buildOutTemplate(self): 
        ...
        #self.emrtFile.create_group(self.emrtFile.root, 'analysis', title='Data Analysis Files, notebooks, scripts and saved results tables.')

        self.TABLES['CLASS_TABLE_MAPPINGS']=self.emrtFile.create_table(self.emrtFile.root,'class_table_mapping', ClassTableMappings, title='Mapping of ioHub DeviceEvent Classes to ioHub DataStore Tables.')

        self.emrtFile.create_group(self.emrtFile.root, 'data_collection', title='Data Collected using the ioHub Event Framework.')
        self.flush()

        self.emrtFile.create_group(self.emrtFile.root.data_collection, 'events', title='All Events that were Saved During Experiment Sessions.')

        self.emrtFile.create_group(self.emrtFile.root.data_collection, 'condition_variables', title="Tables created to Hold Experiment DV and IV's Values Saved During an Experiment Session.")
        self.flush()

        
        self.TABLES['EXPERIMENT_METADETA']=self.emrtFile.create_table(self.emrtFile.root.data_collection,'experiment_meta_data', ExperimentMetaData, title='Information About Experiments Saved to This ioHub DataStore File.')
        self.TABLES['SESSION_METADETA']=self.emrtFile.create_table(self.emrtFile.root.data_collection,'session_meta_data', SessionMetaData, title='Information About Sessions Saved to This ioHub DataStore File.')
        self.flush()


        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'experiment', title='Experiment Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'keyboard', title='Keyboard Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'mouse', title='Mouse Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'touch', title='Touch Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'gamepad', title='GamePad Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'analog_input', title='AnalogInput Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'eyetracker', title='EyeTracker Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'mcu', title='MCU Device Events.')
        self.emrtFile.create_group(self.emrtFile.root.data_collection.events, 'serial', title='Serial Interface Events.')
        self.flush()
    ...
    def _initializeConditionVariableTable(self,experiment_id,session_id,np_dtype):
        ...
        try:
            ...
            experimentConditionVariableTable=self.emrtFile.root.data_collection.condition_variables._f_get_child(expCondTableName)
            self.TABLES['EXP_CV']=experimentConditionVariableTable
        except NoSuchNodeError, nsne:
            try:
                experimentConditionVariableTable=self.emrtFile.create_table(self.emrtFile.root.data_collection.condition_variables,expCondTableName,self._EXP_COND_DTYPE,title='Condition Variable Values for Experiment ID %d'%(experiment_id))
                self.TABLES['EXP_CV']=experimentConditionVariableTable
                self.emrtFile.flush()
            except Exception:
